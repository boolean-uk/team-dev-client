...
const ExerciseView = () => {
  ...
  const handleBackClick = () => navigate('/exercise')
  ...
  return (
    <>
      <Button className="Back-btn__exercises" size='large' variant='outlined' onClick={handleBackClick}>Back</Button>
      <div className="exercise">
        <h1>{currentExercise.name}</h1>
        <h3 style={{ textAlign: 'start' }}>Learning Objectives:</h3>
        <div>
          {currentExercise.objectives.map(o => {
            return (
        ...
        <div className="log-list">
          {logList &&
            logList.map(l => {
              return (
                <p key={l.id} onClick={() => handleClick(l)}>
                  {l.date}
                </p>
              );
            })}
        </div>
        <Link href={currentExercise.gitHubUrl} underline="hover">
